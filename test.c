...
void compile_and_execute(char* filename, char* input_dir, char* answer_dir, char* target_src, int time_limit) {
    ...
    if (pid == -1) {
        ...
        snprintf(execute_command, sizeof(execute_command), "./%s.out < %s", target_src, pathname);
        ...
        if ((output_file = fopen(output_path, "r")) == NULL || (answer_file = fopen(answer_path, "r")) == NULL) {
        ...
        int correct = 1;
        int ch1, ch2;
        while ((ch1 = fgetc(output_file)) != EOF && (ch2 = fgetc(answer_file)) != EOF) {
            if (ch1 != ch2) {
                correct = 0;
                break;
            }
